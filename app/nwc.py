...
async def nwc_handle_instruction(safebox_found: RegisteredSafebox, instruction_obj, evt: Event):
    ...
    if instruction_obj['method'] == 'pay_invoice':
        ...
        transmittal_kind = _coerce(parsed_result['values'].get("transmittal_kind"), settings.RECORD_TRANSMITTAL_KIND)
        transmittal_relays = _coerce_relays(parsed_result['values'].get("transmittal_relays"), settings.RECORD_TRANSMITTAL_RELAYS)
